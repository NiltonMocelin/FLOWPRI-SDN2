...
INTH1=$(ip -br addr show to 172.16.4.40 | awk '{print $1}')
# interface que conecta com C1
INTC1=$(ip -br addr show to 172.16.4.20 | awk '{print $1}')
# interface que conecta com Switches numeros maiores
INTSMA=$(ip -br addr show to 172.16.4.60 | awk '{print $1}')
# interface que conecta com Switches numeros menores
INTSME=$(ip -br addr show to 172.16.4.50 | awk '{print $1}')

echo "Ajustando rotas ip"

# rota para o controlador
sudo ip route add 172.16.4.10 dev $INTC1 metric 1050

# rota para host 
sudo ip route add 172.16.4.30 dev $INTH1 metric 1050
...
sudo ovs-vsctl add-br switch \
         -- set bridge switch other-config:datapath-id=0000000000000004 \
         ...
         -- set-controller switch tcp:172.16.4.10:6653 
