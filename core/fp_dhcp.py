...
# NAO ESTA 100%, ELE PASSA O IP, PORÉM O HOST NAO FICA COMPLETAMENTE CONFIGURADO...
# o host fica enviando arps, mesmo para hosts de outras subredes

### OBS: PARA FUNCIONAR :: refs: https://www.cyberciti.biz/faq/howto-linux-configuring-default-route-with-ipcommand/
# configurar o gateway padrão e adicionar a entrada do gateway na arp list
# remover gateway anterior: ip route del default
# adicionar novo (ver ip do controllador):  ip route add default via 192.168.255.100 dev h1-eth0
# adicionar entrada arp (ver mac do controlador): arp -s 192.168.255.100 ae:a5:3b:d9:bb:d1
# OBS2: em fato, não importa o mac nem o ip do controlador, pois ele não é um roteador, vai tratar como packet-in...



# https://github.com/ahill00/ryu-dhcp/blob/master/dhcp.py
...
def handle_dhcp_discovery(controller_obj, datapath, in_port, dhcp_pkt):
    ...
    # yiaddr = controller_obj._LIST_IPS_DHCP.pop()
    ...
    client_ip_addr = str(controller_obj._LIST_IPS_DHCP.pop())
    ...
    controller_obj.logger.info("Send DHCP message type %s" %
                    (dhcp_msg_type_code[ord(dhcp_offer_msg_type)]))


    msg_option = dhcp.option(tag=dhcp.DHCP_MESSAGE_TYPE_OPT,
                             value= bytearray(dhcp_offer_msg_type, "utf-8"))
    ...
    dhcp_pkt = dhcp.dhcp(hlen=hlen,
                        ...
                        xid=dhcp_pkt.xid,
                        options=options)
    print("Enviando HandleDiscovery")
    # _send_dhcp_packet(controller_obj, datapath, dhcp_pkt, controller_obj.MACc, controller_obj.IPCv4,  in_port) # esse funcionava
    _send_dhcp_packet(controller_obj, datapath, dhcp_pkt, controller_obj.MACc, controller_obj.IPCv4, dhcp_pkt.chaddr, client_ip_addr, in_port)
...
def handle_dhcp_request(controller_obj, dhcp_pkt, datapath, port):
    ...
    controller_obj.logger.info("Send DHCP message type %s" %
                    (dhcp_msg_type_code[ord(dhcp_ack_msg_type)]))
    ...
    time_option = dhcp.option(tag=dhcp.DHCP_IP_ADDR_LEASE_TIME_OPT,
                              value=bytearray('\xFF\xFF\xFF\xFF', "utf-8"))
                            #   value='\xFF\xFF\xFF\xFF')
    msg_option = dhcp.option(tag=dhcp.DHCP_MESSAGE_TYPE_OPT,
                             value=bytearray(dhcp_ack_msg_type, "utf-8"))
    ...
    #  arrumando aqui -- options
    dhcp_pkt = dhcp.dhcp(op=dhcp.DHCP_BOOT_REPLY,
                         hlen=hlen,
                         chaddr=dhcp_pkt.chaddr,
                         siaddr=controller_obj.IPCc, # coloquei agora
                         boot_file=dhcp_pkt.boot_file,# coloquei agora
                         yiaddr=client_ip_addr,
                         giaddr=dhcp_pkt.giaddr,
                         xid=dhcp_pkt.xid,
                         options=options)
    ...
    _send_dhcp_packet(controller_obj, datapath, dhcp_pkt, controller_obj.MACc, controller_obj.IPCv4, dhcp_pkt.chaddr, client_ip_addr, port)


def _send_dhcp_packet(controller_obj, datapath, dhcp_pkt, mac_src, ip_src, mac_dst, ip_dst, in_port):
        # request
    ...
    pkt.add_protocol(ipv4.ipv4
                     (src=ip_src, dst=ip_dst, proto=17))
...
def handle_dhcp(controller, dhcpPkt, datapath, in_port:int):
    #verificar o tipo da mensagem
    msgType = ord(dhcpPkt.options.option_list[0].value) 

    print("ipcv4" , controller.IPCv4)

    # print(msgType)
    # print(dhcpPkt.__dict__)
    if msgType == dhcp.DHCP_DISCOVER:
        # print( 'TIPO111111111111111')
        handle_dhcp_discovery(controller, datapath, in_port, dhcpPkt)
    elif msgType == dhcp.DHCP_REQUEST:
        # print( '22222222222222222222')
        handle_dhcp_request(controller, dhcpPkt, datapath, in_port)
    else: # se for icmp ping request ou reply, configurar uma regra best effort para icmp utilizando a fila de controlle (isso vai ser util para verificar a alcançabilidade dos hosts depois)
        pass
