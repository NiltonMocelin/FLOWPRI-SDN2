...
#    CONTROLLER_IP_HOST = '172.16.10.1' # assim nao funcionou
    ...
    # Create nodes
    h1 = net.addHost( 'h1', mac='8e:49:d6:d3:1e:df', ip='172.16.0.1/24') #cpu=.5/hosts,
    h2 = net.addHost( 'h2', mac='0e:7b:ac:84:a3:69', ip='172.16.0.2/24') #cpu=.5/hosts,
    h3 = net.addHost( 'h3', mac='ae:f4:cf:ab:4c:15', ip='172.16.0.3/24') #cpu=.5/hosts,
    h4 = net.addHost( 'h4', mac='b2:2c:c7:c1:72:a4', ip='172.16.0.4/24') #cpu=.5/hosts,
    ...
    intfh1 = net.addLink(h1, s1, port2=1, bw=100, delay='10ms', loss=0, max_queue_size=1000, use_htb=True).intf1
    intfh2 = net.addLink(h2, s1, port2=2, bw=100, delay='10ms', loss=0, max_queue_size=1000, use_htb=True).intf1
    intfh3 = net.addLink(h3, s1, port2=3, bw=100, delay='10ms', loss=0, max_queue_size=1000, use_htb=True).intf1

    intfh4 = net.addLink(h4, s3, port2=1, bw=100, delay='10ms', loss=0, max_queue_size=1000, use_htb=True).intf1

    net.addLink(s1, s2, port1=4, port2=2, bw=100, delay='10ms', loss=0, max_queue_size=1000, use_htb=True)
    net.addLink(s2, s3, port1=3, port2=4, bw=100, delay='10ms', loss=0, max_queue_size=1000, use_htb=True)
    

    # #Criar um host no root namespace e linkar com o switch -- sobe o controlador, mas os hosts nao enxergam
    root1 = net.addHost('root1', inNamespace=False, ip='10.0.0.8/24')
